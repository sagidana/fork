...
def extract_destination(string):
    ...
    try:
        ...
        if not path.isfile(file_path): return None, 0, 0
    except Exception as e: elog(f"[!] extract_destination: {e}")

    if file_line is None: file_line = 0
    if file_col is None: file_col = 0
